...
def carregar_colaboradores() -> pd.DataFrame:
    try:
        ...
        if not df.empty:
            df['nome_normalizado'] = df['nome'].apply(lambda x: remover_acentos(x).lower())
        return df
...
def salvar_escala_semanal(nome: str, data_inicio: date, horarios: list) -> bool:
    try:
        for i, horario in enumerate(horarios):
            ...
            supabase.rpc('save_escala_dia_final', {'p_nome': nome, 'p_data': data_dia.strftime('%Y-%m-%d'), 'p_horario': horario}).execute()
        return True
...
def aba_consultar_escala_publica(df_colaboradores: pd.DataFrame, df_escalas: pd.DataFrame):
    ...
    if nome_selecionado:
        with st.container(border=True):
            ...
            # CORREÇÃO: Filtra usando o nome normalizado para garantir a correspondência
            nome_selecionado_normalizado = remover_acentos(nome_selecionado).lower()
            resultados = df_escalas[
                (df_escalas["nome_normalizado"] == nome_selecionado_normalizado) &
...
def aba_editar_escala_semanal(df_colaboradores: pd.DataFrame, df_escalas: pd.DataFrame, semanas_iniciadas: list[date]):
    with st.container(border=True):
        ...
        if colaborador and semana_selecionada:
            ...
            # CORREÇÃO: Filtra usando o nome normalizado para garantir a correspondência
            colaborador_normalizado = remover_acentos(colaborador).lower()
            escala_semana_colab = df_escalas[
                (df_escalas['nome_normalizado'] == colaborador_normalizado) &
            ...
            for i in range(7):
                ...
                with cols[i]:
                    horario_selecionado = st.selectbox(dia_str, options=HORARIOS_PADRAO, index=index_horario, key=f"horario_{colaborador_normalizado}_{semana_selecionada.strftime('%Y%m%d')}_{i}")
...
def main():
    ...
    if st.session_state.logado:
        ...
        with tab1: aba_gerenciar_semanas(semanas_iniciadas)
        with tab2: aba_editar_escala_semanal(df_colaboradores, df_escalas, semanas_iniciadas)
        with tab3: aba_gerenciar_colaboradores(df_colaboradores)
        with tab4: aba_consultar_escala_publica(df_colaboradores, df_escalas)
    else:
